...
In the rest of this report, I will begin by looking at the data, showing my model, showing results, followed by a discussion, and then a conclusion. 
...
Many entries from the data are from before the yaer 1950; the dataset actualyl contains 10,418 entries from films before 1950. Films from before 1950 are different from the films that we are used to seeing in today's age in many different ways. Reviewers may have focused on stylistic traits, rather than both stylistic and narrative traits like we see in today's movies; movies were also shorter. Due to the unfair ground we may be measuring the films from different eras, I decided to focus exclusively on films released after 1950's as they conform to the style we are used to seeing in modern films. 
...
```
Judging by the scatterplot, we can see that there is not a clear relation between the variables. The points seem to be scattered randomly without a clear direction. By further analyzing the linear regression summary, we can see that we have a large p-value and can conclude that there is no relation between a film's reception and budget. 
